...
All notable changes to this project are documented in this file. Releases are dated — see CONTRIBUTING.md, "Versioning and releases".

## [Unreleased]

### Changed

- **The field app now sends `X-API-Key` on its Node-RED calls, not only on uploads.** Node-RED gates every endpoint with that header when its `API_KEY` is set, and the app sent it only to the import service — so enabling the gateway guard would have broken the app's reads (`/checklist`, `/findings/open`, `/inspectionProvider`, `/specialties`, `/inspectors`, `/location`) and its canonical calls (`/importCanonical`, `/importFollowUps`). All eight now carry it, read from the same single stored key (`api-key.json`, prompted for on the first upload), so one prompt covers both services; the deployment must set `compliance_flow`'s `API_KEY` and `compliance_import`'s `IMPORT_API_KEY` to the same value. With no key stored the request goes out bare, which is what an unguarded development stack expects. Verified with the guard enabled on a live stack: bare request `401`, correct key `200`, wrong key `401`; the five `fileServices` fetch assertions were updated for the new headers argument, and 473 tests pass.

- **README points at the whole-platform demo quickstart, which lives in `atrocore-docker`.** The app is offline-first and runs without a backend, but exercising an upload end to end needs the platform: §7 of `atrocore-docker/docs/COMPLIANCE_INTEGRATION_RUNBOOK.md` — executable as `atrocore-docker/scripts/demo-quickstart.sh` — is now the single documented clean-clone-to-demonstrable sequence, and the README says where to find it and which endpoints to point `app.config.json` at. No code changed.
...
- **README clarifies version identity.** Documented that the artifact version is `package.json`'s `version` (`1.0.0a`, stamped by electron-builder), `app.config.json`'s `app.name`/`app.version` are display-only, `productName` is `Aviation Safety Oversight Checklist`, and the historical `x.y.z` CHANGELOG labels predate the CalVer scheme.
