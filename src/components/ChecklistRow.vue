  ...
  <tr>
    ...
    <td class="evidence">
      <div class="upload-buttons">
        ...
        />
        <input type="image" :src="fileUpload" height="30" width="30" :disabled="sessionStore.summary.finalized" @click="evidenceUpload(qnumber)" />
        <input type="image" :id="`cameraInput-${qnumber}`" :src="cameraIcon" height="30" width="30" :disabled="sessionStore.summary.finalized" @click="openCamera" />
      </div>
      <table class="preview"  :id="`evidencetable-${qnumber}`">
        <tr v-for="(evidence, index) in session.evidence" :key="index">
          ...
          <td :class="{ 'missing' : (evidenceStore.files[evidence]?.URL == '')}">
            <a :href="evidenceStore.files[evidence]?.URL" download target="_blank">{{ evidenceStore.files[evidence]?.count }}{{
               evidence
            }}</a>
          </td>
        </tr>
...
const capturePhoto = async () => {
  ...
  const file = new File([blob], `evidence-${props.qnumber}-${timestamp}.jpg`, { type: 'image/jpeg' });
  const table = props.session.evidence || [];
  await evidenceStore.add(sessionStore.summary.specialty, file);
