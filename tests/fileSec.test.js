...
describe('fileSec', () => {
  ...
  describe('safeJoin', () => {
    ...
    it('throws on path traversal', () => {
      expect(() => safeJoin('/base', ['../file.txt'])).toThrow('safeJoin: Illegal path name: /base,../file.txt');
      expect(() => safeJoin('/base/../base2', ['file.txt'])).toThrow('safeJoin: Illegal path name: /base/../base2,file.txt');
    });
    ...
    it('throws on non-string base', () => {
      expect(() => safeJoin(123, ['sub','file.txt'])).toThrow('safeJoin: Illegal path name: 123 ; sub,file.txt');
      expect(() => safeJoin(null, ['sub','file.txt'])).toThrow('safeJoin: Illegal path name: null ; sub,file.txt');
      expect(() => safeJoin(undefined, ['sub','file.txt'])).toThrow('safeJoin: Illegal path name: undefined ; sub,file.txt');
    });
    ...
    it('throws on non-string input', () => {
      ...
      expect(() => safeJoin('/base', undefined)).toThrow('safeJoin: Illegal path name: /base ; undefined');
    });
  });
});
